...
class Solution:
    def robotSim(self, commands, obstacles):
        ...
        for i in obstacles:
            x = i[0]
            y = i[1]
            if oDictx.get(x) is None:
                oDictx[x] = [y]
            else:
                oDictx[x].append(y)
            if oDicty.get(y) is None:
                oDicty[y] = [x]
            else:
                oDicty[y].append(x)

        for x in oDictx.keys():
            oDictx[x] = sorted(oDictx[x])
        for y in oDicty.keys():
            oDicty[y] = sorted(oDicty[y])
            ...
            if c == -1:
                ...
                if d == 0:
                    ...
                    if oy is None:
                        ...
                        for i in range(len(oy) - 1, -1, -1):
                            if oy[i] < cy:
                                break
                        if cy >= oy[i] and oy[i] >= nexty:
                            cy = oy[i] + 1
                        else:
                            cy = nexty
                else:
                    ...
                    if ox is None:
                        ...
                        for i in range(len(ox) - 1, -1, -1):
                            if ox[i] < cx:
                                break
                        if cx >= ox[i] and ox[i] >= nextx:
                            cx = ox[i] + 1
                        else:
                            cx = nextx
                dd = cx ** 2 + cy ** 2
...
if __name__ == '__main__':
    ...
    print(s.robotSim(
        [-2,8,3,7,-1],
        [[-4,-1],[1,-1],[1,4],[5,0],[4,5],[-2,-1],[2,-5],[5,1],[-3,-1],[5,-3]])) # 324
    print(s.robotSim(
